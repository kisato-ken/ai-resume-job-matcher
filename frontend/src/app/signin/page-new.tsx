...
export default function SignIn() {
  ...
  const handleGoogleSignIn = async () => {
    setIsLoading(true);
    try {
      // For now, use the demo Google sign-in from AuthContext
      await signInWithGoogle();
      router.push('/analyzer');
    } catch (error) {
      console.error('Google sign-in error:', error);
      alert('Google OAuth demo completed! In production, this would use real Google authentication.');
      router.push('/analyzer');
    } finally {
      setIsLoading(false);
    }
              ...
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <p className="text-sm text-blue-700">
                    Currently in demo mode. Any email/password will work for testing.
                    Google OAuth requires Google Cloud setup (guide included).
                  </p>
                </div>
              </div>
            </div>
